...
if($__TYPE__ > 1){
    ...
    $_query_ = mysqli_query($__CONEXAO__, "select * from equipes where id='$decEquipe' and id in (select equipe from alunos where email='$__EMAIL__')") or die("2");
}
...
while($_dados_ = mysqli_fetch_array($_query_)){
    ...
    $query = mysqli_query($__CONEXAO__, "select id, nome from users where email in (select email from alunos where equipe='$id' and active='1')");
    $alunosArr = array();
