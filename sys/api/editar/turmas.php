...
if($__TYPE__ == 2){
    $checkTurma= mysqli_query($__CONEXAO__, "select id from professores where email='$__EMAIL__' and turma='$id'") or die("b");
    if(mysqli_num_rows($checkTurma) < 1){
...
for($i = 0; $i < count($aulas); $i++){
    ...
    for($j = 0; $j < count($chamadaAula); $j++){
        ...
        if(mysqli_num_rows($check_query) > 0){
            $checkChamada = mysqli_query($__CONEXAO__, "select id from chamada where id='$idChamada' and presenca='$checkAula'")  or endCode("asd 2", false);
            if(mysqli_num_rows($checkChamada) == 0){
                mysqli_query($__CONEXAO__, "update chamada set presenca='$checkAula' where id='$idChamada'")  or endCode("asd 3", false);
            }
        }
    }
}
